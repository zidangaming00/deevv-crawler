...
class MassiveWebCrawler:
  ...
  def __init__(self, start_urls, max_pages=50, concurrency=10):
  ...
  async def process_page(self, url, html):
    ...
    snippet = (
        self.clean_text(text_content)[:200] + '...' if text_content else ''
    )
  ...
  async def run_crawler(self):
    ...
    async with aiohttp.ClientSession(
        headers={'User-Agent': 'MassiveSpiderBot/1.0'}
    ) as session:
      tasks = [
          asyncio.create_task(self.worker(session))
          for _ in range(self.concurrency)
      ]
      # Menunggu semua worker selesai tanpa memakai queue.join() yang rentan deadlock
